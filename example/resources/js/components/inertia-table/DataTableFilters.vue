                ...
                <div class="grid gap-4 py-2">
                    <div v-for="filter in filters" :key="filter.key" class="grid gap-2">
                        ...
                        <div v-else-if="filter.type === 'boolean'" class="flex items-center space-x-2 pt-1">
                            ...
                            />
                            <Label :for="`filter-${filter.key}`" class="font-normal">{{ filter.label || 'Enable' }}</Label>
                        </div>
                    </div>
                </div>

                <div v-if="activeFilterCount > 0" class="flex justify-end pt-2 border-t">
                    <Button type="button" variant="ghost" size="sm" @click="resetFilters" class="text-muted-foreground h-8 px-2 lg:px-3" aria-label="Reset filters">
                        Reset Filters
